...
int cpsw_net_debug = 0;

u8_t NetBcastAddr[6] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };

extern void v7_dma_map_area(void *, size_t, int);
extern void v7_dma_unmap_area(void *, size_t, int);

#define CTRL_DEVICE_BASE		(AM33XX_L4_WK_IO_ADDRESS(0x44E10600))
...
static inline u32 wait_for_user_access(void)
{
	...
	if (timeout == -1) {
		printk("wait_for_user_access Timeout\n");
		return -ETIMEDOUT;
	}
	return reg;
}
			...
			void *buffer, int len)
{
	...
	/* dma地址 */
	desc_dma = (u32)desc - OMAP2_L4_FAST_IO_OFFSET;
	buffer_dma = (u32)buffer - 0xC0000000 + 0x80000000;

	desc_write(desc, hw_next,   0);
	desc_write(desc, hw_buffer, buffer_dma);
			 ...
			 void **buffer, int *len)
{
	...
	u32 desc_dma = (u32)desc - OMAP2_L4_FAST_IO_OFFSET;
	u32 status;
	...
	if (desc_read_ptr(desc, hw_next) == NULL)
		chan->head = NULL;
	else
		chan->head = desc_read_ptr(desc, hw_next) + OMAP2_L4_FAST_IO_OFFSET;
...
static int cpsw_initialize(struct dim_sum_netdev *netdev)
{
	struct cpsw_priv	*priv = netdev->priv;
	struct cpsw_slave	*slave;
	int i, ret;
	u32 reg;

	reg = __raw_readl(&priv->regs->id_ver);
	printk("Initializing cpsw driver version %d.%d (%d)\n", (reg >> 8 & 0x7), reg & 0xff, (reg >> 11) & 0x1f);
...
static void cpsw_halt_netdev(struct dim_sum_netdev *netdev)
{
	struct cpsw_priv	*priv = netdev->priv;
...
static int cpsw_send_pkt(struct dim_sum_netdev *netdev, void *packet, int length)
{
	struct cpsw_priv	*priv = netdev->priv;
	void *buffer;
	int len;
	...
	if (cpsw_net_debug == 1) {
		...
		printk("\n$$$$$$ %s, packet:%x, len:%d\n", __FUNCTION__, (u32)packet, length);
		for (i=0;i<length; i++)
			printk("%x ", *p++);
		printk("\n\n");
	}
	...
	v7_dma_map_area(packet, length, DMA_TO_DEVICE);
...
static int cpsw_recv_pkt(struct dim_sum_netdev *netdev, void *packet, int *length)
{
	struct cpsw_priv	*priv = netdev->priv;
	void *buffer;
	int len;
	...
	if ((ret = cpdma_process(priv, &priv->rx_chan, &buffer, &len)) >= 0) {
		if (cpsw_net_debug == 1) {
			int i;
			char *ptr = buffer;

			printk(" \n$$$$$$ %s, buffer:%x, len:%d\n", __FUNCTION__, (u32)buffer, len);
			for (i = 0; i<len; i++)
				printk("%x ", *ptr++);
			printk("\n\n");
		}
		...
		v7_dma_unmap_area(buffer, len, DMA_FROM_DEVICE);
...
static int cpsw_register(struct cpsw_platform_data *data)
{
	struct cpsw_priv	*priv;
	struct cpsw_slave	*slave;
	void	*regs = (void *)data->cpsw_base;
	struct dim_sum_netdev *cpsw_net;
	struct in_addr tmpaddr;
	int idx = 0, i;

	cpsw_net = dim_sum_mem_alloc(sizeof(struct dim_sum_netdev), MEM_NORMAL);
	if (!cpsw_net) {
		printk("cpsw_net_init, ");
		return -1;
	}
	memset(cpsw_net, 0, sizeof(*cpsw_net));

	priv = dim_sum_mem_alloc(sizeof(*priv), MEM_NORMAL);
	if (!priv) {
		dim_sum_mem_free(cpsw_net);
		return -ENOMEM;
	}
	...
	priv->slaves = dim_sum_mem_alloc(sizeof(struct cpsw_slave) * data->slaves, MEM_NORMAL);
	if (!priv->slaves) {
		dim_sum_mem_free(cpsw_net);
		dim_sum_mem_free(priv);
		return -ENOMEM;
	}
	...
	for (i = 0; i < PKTBUFSRX; i++) {
		NetRxPackets[i] = dim_sum_mem_alloc(PKTSIZE_ALIGN + PKTALIGN, MEM_NORMAL);
		if (!NetRxPackets[i]) {
			printk("## dim_sum_mem_alloc fail, %s, %d\n", __FUNCTION__, __LINE__);
			return -ENOMEM;
		}
	}

	strcpy(cpsw_net->name, "cpsw");
	cpsw_net->initialize = cpsw_initialize;
	cpsw_net->send_pkt = cpsw_send_pkt;
	cpsw_net->recv_pkt = cpsw_recv_pkt;
	cpsw_net->halt_netdev = cpsw_halt_netdev;
	cpsw_net->priv = priv;

	memcpy(cpsw_net->enetaddr, mac_addr, sizeof(mac_addr));

	/** 初始化IP地址为192.168.1.100/24 **/
	inet_aton("192.168.1.100", &tmpaddr);
	cpsw_net->ipaddr.addr = tmpaddr.s_addr;
	inet_aton("255.255.255.0", &tmpaddr);
	cpsw_net->netmask.addr = tmpaddr.s_addr;	

	dim_sum_netdev_register(cpsw_net);

	return 1;

}

int cpsw_net_init(void)
{
	int ret;
	...
	writel(RMII_MODE_ENABLE | RMII1_IO_CLK_EN, &cdev->miisel);
	cpsw_slaves[0].phy_if = cpsw_slaves[1].phy_if =	PHY_INTERFACE_MODE_RMII;

	ret = cpsw_register(&cpsw_data);
	if (ret < 0)
		printk("Error %d registering CPSW switch\n", ret);
