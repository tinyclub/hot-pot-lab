...
//extern unsigned int omap_timer_get_last_time(void);
#if 0
...
{	
	...
	printk("part_num %d capacity %llx\n", part_num, mmc->capacity);
	//mmc->block_dev.lba = lldiv(mmc->capacity, mmc->read_bl_len);
	mmc->block_dev.lba = div_u64(mmc->capacity, mmc->read_bl_len);
	printk("read_bl_len %x lba %x\n", mmc->read_bl_len, mmc->block_dev.lba);
	return 0;
}
...
static int mmc_startup(struct mmc *mmc)
{
	...
	//mmc->block_dev.lba = lldiv(mmc->capacity, mmc->read_bl_len);
	mmc->block_dev.lba = div_u64(mmc->capacity, mmc->read_bl_len);
	printk("capacity %llx read_bl_len %x\n", mmc->capacity, mmc->read_bl_len);
	//sprintf(mmc->block_dev.vendor, "Man %06x Snr %04x%04x",
	printk("Man %06x Snr %04x%04x\n",
...
int mmc_do_init(struct mmc *mmc)
{
	int err = IN_PROGRESS;
...
int mmc_init(void)
{
	...
	if (!mmc) {
		...
		printk("spl: mmc device not found!!\n");
		return -1;
	}
