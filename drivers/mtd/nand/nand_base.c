...
#include <dim-sum/mtd/nand.h>
#include <dim-sum/mtd/nand_ecc.h>
...
{
	SysSemMTake(this->chip_lock,MAX_SCHEDULE_TIMEOUT); 
...
static int nand_wait(struct mtd_info *mtd, struct nand_chip *this)
{
	...
	if (state == FL_ERASING)
		timeo = (CONFIG_HZ * 400) / 1000;
	else
		timeo = (CONFIG_HZ * 20) / 1000;
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	if (!(chip->options & NAND_OWN_BUFFERS))
		chip->buffers = dim_sum_mem_alloc(sizeof(*chip->buffers), MEM_NORMAL);
	if (!chip->buffers)
		return -ENOMEM;
...
void nand_release(struct mtd_info *mtd)
{
	...
	dim_sum_mem_free(chip->bbt);
	if (!(chip->options & NAND_OWN_BUFFERS))
		dim_sum_mem_free(chip->buffers);
}
