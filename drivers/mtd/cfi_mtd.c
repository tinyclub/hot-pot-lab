	...
	size_t *retlen, u_char *buf)
{
	...
	u_char *f = (u_char*)(fi->start[0]) + from;
        ker_semBTake((SEM_ID)fi->flsem, 0xffffffff);
	memcpy_fromio(buf, f, len);
        ker_semBGive((SEM_ID)fi->flsem);
	*retlen = len;
...
static int cfi_mtd_set_erasesize(struct mtd_info *mtd, flash_info_t *fi)
{
	...
	mtd->eraseregions = dim_sum_mem_alloc(sizeof(struct mtd_erase_region_info) * regions, MEM_NORMAL);
