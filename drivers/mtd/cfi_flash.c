...
#if CONFIG_HZ != 1000
	if ((ulong)CONFIG_HZ > 100000)
		tout *= (ulong)CONFIG_HZ / 1000;  /* for a big HZ, avoid overflow */
	else
		tout = DIV_ROUND_UP(tout * (ulong)CONFIG_HZ, 1000);
#endif

	/* Wait for command completion */
	start = SysUpTime ();
	while (flash_is_busy (info, sector)) {
		if ((ulong)(SysUpTime () -start )> tout*1000000) {
			printk ("Flash %s timeout at address %lx data %lx\n",
				    ...
				    ulong tout, char *prompt)
{
	...
	case CFI_CMDSET_INTEL_STANDARD:
		if ((retcode != ERR_OK)
			...
			retcode = ERR_INVAL;
			printk ("Flash %s error at address %lx\n", prompt,
				info->start[sector]);
			if (flash_isset (info, sector, 0, FLASH_STATUS_ECLBS |
					 FLASH_STATUS_PSLBS)) {
				printk ("Command Sequence Error.\n");
			} else if (flash_isset (info, sector, 0,
						FLASH_STATUS_ECLBS)) {
				printk ("Block Erase Error.\n");
				retcode = ERR_NOT_ERASED;
			} else if (flash_isset (info, sector, 0,
						FLASH_STATUS_PSLBS)) {
				printk ("Locking Error\n");
			}
...
void flash_print_info (flash_info_t * info)
{
	...
	printk ("%s FLASH (%d x %d)",
		info->name,
		(info->portwidth << 3), (info->chipwidth << 3));
	if (info->size < 1024*1024)
		printk ("  Size: %ld kB in %d Sectors\n",
			info->size >> 10, info->sector_count);
	else
		printk ("  Size: %ld MB in %d Sectors\n",
			info->size >> 20, info->sector_count);
	printk ("  ");
	switch (info->vendor) {
		case CFI_CMDSET_INTEL_PROG_REGIONS:
			printk ("Intel Prog Regions");
			break;
		case CFI_CMDSET_INTEL_STANDARD:
			printk ("Intel Standard");
			break;
		case CFI_CMDSET_INTEL_EXTENDED:
			printk ("Intel Extended");
			break;
		case CFI_CMDSET_AMD_STANDARD:
			printk ("AMD Standard");
			break;
...
#endif
		default:
			printk ("Unknown (%d)", info->vendor);
			break;
	}
	printk (" command set, Manufacturer ID: 0x%02X, Device ID: 0x%02X",
		info->manufacturer_id, info->device_id);
	if (info->device_id == 0x7E) {
		printk("%04X", info->device_id2);
	}
	printk ("\n  Erase timeout: %ld ms, write timeout: %ld ms\n",
		info->erase_blk_tout,
		info->write_tout);
	if (info->buffer_size > 1) {
		printk ("  Buffer write timeout: %ld ms, "
			"buffer size: %d bytes\n",
		info->buffer_write_tout,
		info->buffer_size);
	}
...
ulong flash_get_size (phys_addr_t base, int banknum)
{
	...
	if (flash_detect_cfi (info, &qry)) {
		...
		case CFI_CMDSET_AMD_EXTENDED:
			...
			break;
		default:
			printk("CFI: Unknown command set 0x%x\n",
					info->vendor);
			/*
		...
		for (i = 0; i < num_erase_regions; i++) {
			if (i > NUM_ERASE_REGIONS) {
				printk ("%d erase regions found, only %d used\n",
			...
			for (j = 0; j < erase_region_count; j++) {
				if (sect_cnt >= CONFIG_SYS_MAX_FLASH_SECT) {
					printk("ERROR: too many flash sectors\n");
					break;
				}
				info->start[sect_cnt] =
...
#define BANK_BASE(i)	(((phys_addr_t [CFI_MAX_FLASH_BANKS])CONFIG_SYS_FLASH_BANKS_LIST)[i])
	...
	for (i = 0; i < CONFIG_SYS_MAX_FLASH_BANKS; ++i) {
		...
		if (flash_info[i].flash_id == FLASH_UNKNOWN) {
			...
			printk ("## Unknown FLASH on Bank %d "
				"- Size = 0x%08lx = %ld MB\n",
