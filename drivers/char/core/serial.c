...
struct async_struct rs_table[] = {
	...
	{ 0, BASE_BAUD, 0xF9E09000, 4, STD_COM_FLAGS },	/* ttyS0 */
};
...
static DECLARE_SEMAPHORE(tmp_buf_sem);
 ...
 * for submarines.
 */
__maybe_unused static void rs_probe(int irq, struct pt_regs * regs)
{
...
static void rs_interrupt(int irq, struct pt_regs * regs)
{
	...
	do {
		...
		info->last_active = get_jiffies_64();
 ...
 */
__maybe_unused static void do_serial_bh(void *unused)
{
...
static void do_softint(void *private_)
{
	...
	if (test_and_clear_bit(RS_EVENT_WRITE_WAKEUP, (unsigned long *)&info->event)) {
 ...
 * CPU overhead).
 */
__maybe_unused static void rs_timer(void)
{
	static unsigned long last_strobe = 0;
	struct async_struct *info;
	unsigned int	i;

	if ((get_jiffies_64() - last_strobe) >= RS_STROBE_TIME*HZ) {
		for (i=1; i < 16; i++) {
			info = IRQ_ports[i];
			if (!info)
				continue;
	...
	last_strobe = get_jiffies_64();
#ifdef xby_debug
	timer_table[RS_TIMER].expires = get_jiffies_64() + RS_STROBE_TIME * HZ;
	...
	if (IRQ_ports[0]) {
		...
		timer_table[RS_TIMER].expires = get_jiffies_64() + IRQ_timeout[0] - 2;
	}
#endif
}
...
static int startup(struct async_struct * info)
{
	...
	if (!info->xmit_buf) {
		info->xmit_buf = (unsigned char *) dim_sum_pages_alloc(0, MEM_NORMAL);
		if (!info->xmit_buf)
			return -ENOMEM;
	}
	...
	if (info->irq && (!IRQ_ports[info->irq] ||
		...
		retval = 0;//request_irq(info->irq, handler, SA_INTERRUPT, "serial");
		if (retval) {
			local_irq_restore(flags);
			if (suser()) {
				if (info->tty)
	 ...
	 */
	timer_table[RS_TIMER].expires = get_jiffies_64() + 2;
	timer_active |= 1 << RS_TIMER;
#endif
	/*
...
static void shutdown(struct async_struct * info)
{
	...
	if (info->irq && (!IRQ_ports[info->irq] ||
		...
		if (IRQ_ports[info->irq]) {
			//free_irq(info->irq);
			retval = 0;//request_irq(info->irq, rs_interrupt_single, SA_INTERRUPT, "serial");
	...
	if (info->xmit_buf) {
		dim_sum_pages_free((void *) info->xmit_buf);
		info->xmit_buf = 0;
	}
...
static void send_break(	struct async_struct * info, int duration)
{
	...
	current->timeout = get_jiffies_64() + duration;
	local_irq_disable();
...
static int check_wild_interrupts(int doprint)
{
	 ...
	 */
	timeout = get_jiffies_64()+10;
	while (timeout >= get_jiffies_64())
		;
...
static void rs_close(struct tty_struct *tty, struct file * filp)
{
	...
	if (info->flags & ASYNC_INITIALIZED) {
		 ...
		 */
		timeout = get_jiffies_64()+HZ;
		while (!(serial_inp(info, UART_LSR) & UART_LSR_TEMT)) {
			current->state = TASK_INTERRUPTIBLE;
			current->timeout = get_jiffies_64() + info->timeout;
			TaskSwitch();
			if (get_jiffies_64() > timeout)
				break;
		}
	}
	shutdown(info);
	...
	if (info->blocked_open) {
		if (info->close_delay) {
			current->state = TASK_INTERRUPTIBLE;
			current->timeout = get_jiffies_64() + info->close_delay;
			TaskSwitch();
		}
		wake_up_interruptible(&info->open_wait);
	}
...
int rs_open(struct tty_struct *tty, struct file * filp)
{
	...
	if (!tmp_buf) {
		tmp_buf = (unsigned char *) dim_sum_pages_alloc(0, MEM_NORMAL);
		if (!tmp_buf)
			return -ENOMEM;
	}
