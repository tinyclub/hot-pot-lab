...
int vc_allocate(unsigned int i)		/* return 0 on success */
{
	...
	if (!vc_cons[i].d) {
	    ...
	    q = (long) dim_sum_mem_alloc(video_screen_size, MEM_NORMAL);
	    if (!q)
	      return -ENOMEM;
	    p = (long) dim_sum_mem_alloc(structsize, MEM_NORMAL);
	    if (!p) {
...
int vc_resize(unsigned long lines, unsigned long cols)
{
	...
	__maybe_unused unsigned long occ, oll, oss, osr;
	unsigned short *p;
	...
	for (currcons = 0; currcons < MAX_NR_CONSOLES; currcons++) {
		...
		p = (unsigned short *) dim_sum_mem_alloc(ss, MEM_NORMAL);
		if (!p) {
...
void con_init(void)
{
	...
	/* Due to dim_sum_mem_alloc roundup allocating statically is more efficient -
	   so provide MIN_NR_CONSOLES for people with very little memory */
	for (currcons = 0; currcons < MIN_NR_CONSOLES; currcons++) {
		vc_cons[currcons].d = AllocPermanentBootMemory(0, sizeof(struct vc_data), 0);
		vt_cons[currcons] = AllocPermanentBootMemory(0, sizeof(struct vt_struct), 0);
		vc_scrbuf[currcons] = AllocPermanentBootMemory(0, video_screen_size, 0);
		kmalloced = 0;
 ...
 */
__maybe_unused static void blank_screen(void)
{
	do_blank_screen(0);
}

__maybe_unused static void unblank_screen(void)
{
	do_unblank_screen();
}
