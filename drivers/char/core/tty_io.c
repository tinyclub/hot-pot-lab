...
static int tty_select(struct inode *, struct file *, int, wait_queue_t *);
...
static int hung_up_tty_select(struct inode * inode, struct file * filp, int sel_type, wait_queue_t * wait)
{
	return 1;
}
...
repeat:
	...
	retval = -ENOMEM;
	if (!*tty_loc && !tty) {
		if (!(tty = (struct tty_struct*) dim_sum_pages_alloc(0, MEM_NORMAL)))
			goto end_init;
	...
	if (!*tp_loc && !tp) {
		tp = (struct termios *) dim_sum_mem_alloc(sizeof(struct termios),
						MEM_NORMAL);
		if (!tp)
			goto end_init;
	...
	if (!*ltp_loc && !ltp) {
		ltp = (struct termios *) dim_sum_mem_alloc(sizeof(struct termios),
						 MEM_NORMAL);
		if (!ltp)
			goto end_init;
	...
	if (driver->type == TTY_DRIVER_TYPE_PTY) {
		...
		if (!*o_tp_loc && !o_tp) {
			o_tp = (struct termios *)
				dim_sum_mem_alloc(sizeof(struct termios), MEM_NORMAL);
			if (!o_tp)
				goto end_init;
...
end_init:
	if (tty)
		dim_sum_pages_free((void *) tty);
	if (o_tty)
		dim_sum_pages_free((void *) o_tty);
	if (tp)
...
static int tty_select(struct inode * inode, struct file * filp, int sel_type, wait_queue_t * wait)
{
	struct tty_struct * tty;
...
static int tty_fasync(struct inode * inode, struct file * filp, int on)
{
	...
	if (on) {
		if (fa)
			return 0;
		fa = (struct fasync_struct *)dim_sum_mem_alloc(sizeof(struct fasync_struct), MEM_NORMAL);
		if (!fa)
			return -ENOMEM;
 ...
 * later.
 */
void console_init(long kmem_end)
{
