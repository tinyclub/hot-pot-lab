...
static inline void n_tty_receive_overrun(struct tty_struct *tty)
{
	...
	tty->num_overrun++;
	if (tty->overrun_time < (get_jiffies_64() - HZ)) {
		printk("%s: %d input overrun(s)\n", _tty_name(tty, buf),
		       tty->num_overrun);
		tty->overrun_time = get_jiffies_64();
		tty->num_overrun = 0;
	}
}
...
static inline void n_tty_receive_char(struct tty_struct *tty, unsigned char c)
{
	 ...
	 * up.
	 */
	if (!test_bit(c, (unsigned long *)&tty->process_char_map) || tty->lnext) {
		finish_erasing(tty);
		tty->lnext = 0;
		if (L_ECHO(tty)) {
	...
	if (L_ICANON(tty)) {
		...
		handle_newline:
			set_bit(tty->read_head, (unsigned long *)&tty->read_flags);
			put_tty_queue(c, tty);
...
static void n_tty_set_termios(struct tty_struct *tty, struct termios * old)
{
	...
	if (I_ISTRIP(tty) || I_IUCLC(tty) || I_IGNCR(tty) ||
		...
		if (I_IGNCR(tty) || I_ICRNL(tty))
			set_bit('\r', (unsigned long *)&tty->process_char_map);
		if (I_INLCR(tty))
			set_bit('\n', (unsigned long *)&tty->process_char_map);

		if (L_ICANON(tty)) {
			set_bit(ERASE_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit(KILL_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit(EOF_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit('\n', (unsigned long *)&tty->process_char_map);
			set_bit(EOL_CHAR(tty), (unsigned long *)&tty->process_char_map);
			if (L_IEXTEN(tty)) {
		...
		if (I_IXON(tty)) {
			set_bit(START_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit(STOP_CHAR(tty), (unsigned long *)&tty->process_char_map);
		}
		if (L_ISIG(tty)) {
			set_bit(INTR_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit(QUIT_CHAR(tty), (unsigned long *)&tty->process_char_map);
			set_bit(SUSP_CHAR(tty), (unsigned long *)&tty->process_char_map);
		}
		clear_bit(__DISABLED_CHAR, (unsigned long *)&tty->process_char_map);
		local_irq_enable();
		tty->raw = 0;
		tty->real_raw = 0;
	} else {
...
static void n_tty_close(struct tty_struct *tty)
{
	n_tty_flush_buffer(tty);
	if (tty->read_buf) {
		dim_sum_pages_free((void *) tty->read_buf);
		tty->read_buf = 0;
	}
}
...
static int n_tty_open(struct tty_struct *tty)
{
	...
	if (!tty->read_buf) {
		tty->read_buf = (unsigned char *)
			dim_sum_pages_alloc(0, MEM_NORMAL);
		if (!tty->read_buf)
			return -ENOMEM;
	}
...
do_it_again:
	...
	if (L_ICANON(tty)) {
		...
		if (minimum) {
			...
			if (time) {
				current->timeout = time + get_jiffies_64();
				time = 0;
			} else
				current->timeout = 0;
...
void select_wait(wait_queue_head_t * wait_address, wait_queue_t * p)
{
#if 0
...
static int normal_select(struct tty_struct * tty, struct inode * inode,
			 struct file * file, int sel_type, wait_queue_t *wait)
{
	switch (sel_type) {
		case SEL_IN:
