...
static int umsdos_waitcreate(struct inode *dir)
{
	...
	if (dir->u.umsdos_i.u.dir_info.creating
		...
		sleep_on(&dir->u.umsdos_i.u.dir_info.dir_wait_q);
		ret = 1;
	}
	return ret;
}
...
void umsdos_unlockcreate (struct inode *dir)
{
	...
	printk("%p\n", dir->u.umsdos_i.u.dir_info.dir_wait_q.task_list.next);
	wake_up (&dir->u.umsdos_i.u.dir_info.dir_wait_q);
}
/*
		...
		if (ret == 0){
			...
			if (ret == 0){
				...
				if (ret == 0){
					...
					PRINTK (("inode %p[%d] ",inode,atomic_read(&inode->i_count)));
	...
	if (S_ISDIR(oldinode->i_mode)){
		...
		*/
		...
		PRINTK (("umsdos_link dir_owner = %d -> %p [%d] "
			,oldinode->u.umsdos_i.i_dir_owner,olddir,atomic_read(&olddir->i_count)));
		if (ret == 0){
				...
				if (!(entry.flags & UMSDOS_HIDDEN)){
					...
					*/
					...
					if (ret == 0){
						...
						PRINTK (("olddir[%d] ",atomic_read(&olddir->i_count)));
						ret = umsdos_rename_f (olddir,entry.name
							,entry.name_len
							,olddir,info.entry.name,info.entry.name_len
							,UMSDOS_HIDDEN);
						if (ret == 0){
							char *path = (char*)dim_sum_mem_alloc(PATH_MAX,MEM_NORMAL);
							if (path == NULL){
								ret = -ENOMEM;
							}else{
								PRINTK (("olddir[%d] ",atomic_read(&olddir->i_count)));
	...
	if (ret == 0){
		...
		if (ret == 0){
			...
			if (atomic_read(&sdir->i_count) > 1){
				...
				PRINTK (("isempty %d i_count %d ",empty,atomic_read(&sdir->i_count)));
				/* check sticky bit */
