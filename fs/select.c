 ...
 * select_wait() is a inline-function defined in <dim-sum/sched.h>, as all select
...
static __maybe_unused void free_wait(select_table * p)
{
...
static __maybe_unused int check(int flag, select_table * wait, struct file * file)
{
#ifdef xby_debug
	struct inode * inode;
...
static __maybe_unused int do_select(int n, fd_set *in, fd_set *out, fd_set *ex,
	...
	for (j = 0 ; j < __FDSET_LONGS ; j++) {
		...
		for ( ; set ; i++,set >>= 1) {
			...
			if (!current->files->fd_array[i])
				return -EBADF;
			if (!current->files->fd_array[i]->f_inode)
				return -EBADF;
			max = i;
		}
	}
end_check:
	n = max + 1;
	if(!(entry = (struct select_table_entry*) dim_sum_pages_alloc(0, MEM_NORMAL)))
		return -ENOMEM;
	FD_ZERO(res_in);
	FD_ZERO(res_out);
	FD_ZERO(res_ex);
...
repeat:
	...
	for (i = 0 ; i < n ; i++) {
		if (FD_ISSET(i,in) && check(SEL_IN,wait,current->files->fd_array[i])) {
			FD_SET(i, res_in);
			count++;
			wait = NULL;
		}
		if (FD_ISSET(i,out) && check(SEL_OUT,wait,current->files->fd_array[i])) {
			FD_SET(i, res_out);
			count++;
			wait = NULL;
		}
		if (FD_ISSET(i,ex) && check(SEL_EX,wait,current->files->fd_array[i])) {
			FD_SET(i, res_ex);
			count++;
			wait = NULL;
		}
	...
	dim_sum_pages_free((unsigned long) entry);
 ...
 */
static __maybe_unused int __get_fd_set(int nr, unsigned long * fs_pointer, unsigned long * fdset)
{
	int error;
...
static __maybe_unused void __set_fd_set(int nr, unsigned long * fs_pointer, unsigned long * fdset)
{
	if (!fs_pointer)
		return;
	while (nr > 0) {
