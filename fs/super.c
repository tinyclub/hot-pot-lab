...
static unsigned long unnamed_dev_in_use[256/sizeof(unsigned long)] = { 0, };
...
static void put_unnamed_dev(dev_t dev)
{
	...
	if (MAJOR(dev) == UNNAMED_MAJOR &&
	    test_and_clear_bit(MINOR(dev), unnamed_dev_in_use))
		return;
...
void mount_root(void)
{
	...
	for (fs_type = file_systems ; fs_type ; fs_type = fs_type->next) {
		...
		if (sb) {
			inode = sb->s_mounted;
			atomic_add(3, &inode->i_count );	/* NOTE! it is logically used 4 times, not 1 */
			sb->s_covered = inode;
...
static __maybe_unused int do_remount(const char *dir,int flags,char *data)
{
	struct inode *dir_i;
	int retval;
...
asmlinkage int sys_umount(const char * name)
{
	struct inode * inode;
	dev_t dev;
	int retval;
