			      ...
			      struct ext2_super_block * es)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		if (!(sb->u.ext2_sb.s_mount_state & EXT2_VALID_FS))
			...
			(es->s_lastcheck + es->s_checkinterval <= FS_CURRENT_TIME))
				      ...
				      int silent)
{
	...
	init_waitqueue_head(&sb->u.ext2_sb.s_rename_wait);
#ifdef EXT2FS_PRE_02B_COMPAT
	...
	sb->u.ext2_sb.s_group_desc = dim_sum_mem_alloc (db_count * sizeof (struct buffer_head *), MEM_NORMAL);
