		       ...
		       unsigned long count)
{
	...
	for (i = 0; i < count; i++) {
		if (!test_and_clear_bit(bit + i, (unsigned long *)bh->b_data))
...
repeat:
	...
	if (gdp->bg_free_blocks_count > 0) {
		...
		if (!test_bit(j, (unsigned long *)bh->b_data)) {
#ifdef EXT2FS_DEBUG
			goal_hits++;
...
search_back:
	 ...
	 */
	for (k = 0; k < 7 && j > 0 && !test_bit (j - 1, (unsigned long *)bh->b_data); k++, j--);
...
got_block:
	...
	if (test_and_set_bit (j, (unsigned long *)bh->b_data)) {
	...
	if (prealloc_block) {
		...
		*prealloc_block = tmp + 1;
		for (k = 1;
			...
			if (test_and_set_bit (j + k, (unsigned long *)bh->b_data))
				break;
			(*prealloc_count)++;
		}	
				...
				unsigned char * map)
{
	return test_bit ((block - sb->u.ext2_sb.s_es->s_first_data_block) %
			 EXT2_BLOCKS_PER_GROUP(sb), (unsigned long *)map);
}
...
void ext2_check_blocks_bitmap (struct super_block * sb)
{
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		if (!test_bit (0, (unsigned long *)bh->b_data))
			ext2_error (sb, "ext2_check_blocks_bitmap",
				    "Superblock in group %d is marked free", i);

		for (j = 0; j < desc_blocks; j++)
			if (!test_bit (j + 1, (unsigned long *)bh->b_data))
				ext2_error (sb, "ext2_check_blocks_bitmap",
