	...
	*err = -ENOSPC;
	while (1) {
		...
		if ((de->inode == 0 && de->rec_len >= rec_len) ||
			 ...
			 */
			dir->i_mtime = dir->i_ctime = FS_CURRENT_TIME;
			dir->i_dirt = 1;
			dir->i_version = ++event;
...
repeat:
	...
	inode->i_dirt = 1;
	dir->i_nlink--;
	inode->i_ctime = dir->i_ctime = dir->i_mtime = FS_CURRENT_TIME;
	dir->i_dirt = 1;
end_rmdir:
	iput (dir);
	iput (inode);
...
repeat:
	...
	dir->i_ctime = dir->i_mtime = FS_CURRENT_TIME;
	dir->i_dirt = 1;
	inode->i_nlink--;
	inode->i_dirt = 1;
	       ...
	       const char * name, int len)
{
	...
	iput (dir);
	oldinode->i_nlink++;
	oldinode->i_ctime = FS_CURRENT_TIME;
	oldinode->i_dirt = 1;
	iput (oldinode);
	return 0;
}
...
start_up:
	...
	if (new_inode) {
		new_inode->i_nlink--;
		new_inode->i_ctime = FS_CURRENT_TIME;
		new_inode->i_dirt = 1;
	}
	old_dir->i_ctime = old_dir->i_mtime = FS_CURRENT_TIME;
	old_dir->i_dirt = 1;
	if (dir_bh) {
